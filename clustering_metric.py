...
class clustering_metrics:
    ...
    def purity_score(self):
        ...
        y_voted_labels = np.zeros(self.true_label.shape)
        # Ordering labels
        ## Labels might be missing e.g with set like 0,2 where 1 is missing
        ## First find the unique labels, then map the labels to an ordered set
        ## 0,2 should become 0,1
        labels = np.unique(self.true_label)
        ordered_labels = np.arange(labels.shape[0])
        for k in range(labels.shape[0]):
            self.true_label[self.true_label==labels[k]] = ordered_labels[k]
        ...
        for cluster in np.unique(self.pred_label):
            hist, _ = np.histogram(self.true_label[self.pred_label==cluster], bins=bins)
            # Find the most present label in the cluster
            winner = np.argmax(hist)
            y_voted_labels[self.pred_label==cluster] = winner

        return accuracy_score(self.true_label, y_voted_labels)
