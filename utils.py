...
def load_npz(dataset):
    ...
    file_map = {'amazon_electronics_photo':'amazon_electronics_photo.npz','coauthor-cs': 'ms_academic_cs.npz', 'coauthor-phy': 'ms_academic_phy.npz'}
    file_name = file_map[dataset]

    with np.load('/content/drive/MyDrive/RWR-GAE/data/MS/' + file_name,allow_pickle=True) as f:
        f = dict(f)
    ...
    y_test[test_mask.astype(int), :] = labels[test_mask.astype(int), :]
    ...
    return adj, features.todense(), y_test, test_mask.astype(int), np.argmax(labels,1)
...
def load_data(dataset):
    ...
    for i in range(len(names)):
        with open("/content/drive/MyDrive/RWR-GAE/gae/data/ind.{}.{}".format(dataset, names[i]), 'rb') as rf:
            u = pkl._Unpickler(rf)
            u.encoding = 'latin1'
            cur_data = u.load()
    ...
    test_idx_reorder = parse_index_file(
        "/content/drive/MyDrive/RWR-GAE/gae/data/ind.{}.test.index".format(dataset))
